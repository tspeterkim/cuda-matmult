...
template <const int TILE_WIDTH, const int COARSE_FACTOR>
__global__ void textbook_coarsed_flipped(int M, int N, int K, float alpha,
                                       const float *A, const float *B,
                                       float beta, float *C) {
...
template <const int TILE_WIDTH, const int COARSE_FACTOR>
__global__ void textbook_coarsed(int M, int N, int K, float alpha,
                                       const float *A, const float *B,
                                       float beta, float *C) {
    __shared__ float Mds[TILE_WIDTH][TILE_WIDTH];
    __shared__ float Nds[TILE_WIDTH][TILE_WIDTH];
...
void run_sgemm_shared_mem_block_coarsed_flipped(int M, int N, int K, float alpha, float *A,
                                float *B, float beta, float *C) {
	if (CEIL_DIV(M,32) < 8) {
		const int coarse_factor = 4;
		dim3 gridDim(CEIL_DIV(M, 32)/coarse_factor, CEIL_DIV(N, 32));
		dim3 blockDim(32, 32);
		textbook_coarsed_flipped<32, coarse_factor>
...
void run_sgemm_shared_mem_block_coarsed(int M, int N, int K, float alpha, float *A,
                                float *B, float beta, float *C) {
	if (CEIL_DIV(M,32) < 8) {
		const int coarse_factor = 4;
		dim3 gridDim(CEIL_DIV(M, 32)/coarse_factor, CEIL_DIV(N, 32));
		dim3 blockDim(32, 32);
		textbook_coarsed<32, coarse_factor>
			<<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
...
void run_sgemm_shared_mem_1d_blocktiling(int M, int N, int K, float alpha, float *A,
                                float *B, float beta, float *C) {
	const int coarse_factor = 8;
  dim3 gridDim(CEIL_DIV(M, 32), CEIL_DIV(N, 32));
  dim3 blockDim(32, 32/coarse_factor);
  sgemm1DBlocktiling<32, coarse_factor>
      <<<gridDim, blockDim>>>(M, N, K, alpha, A, B, beta, C);
...
void invrange_init_matrix(float *mat, int N) {
  for (int i = 0; i < N; i++) {
    mat[i] = N-i;
  }
}
...
void run_kernel(int kernel_num, int M, int N, int K, float alpha, float *A,
  ...
  case 2:
    ...
    break;
  case 3:
    run_sgemm_shared_mem_block_coarsed(M, N, K, alpha, A, B, beta, C);
    break;
  case 4:
    run_sgemm_shared_mem_1d_blocktiling(M, N, K, alpha, A, B, beta, C);
    break;
  case 5:
    run_sgemm_shared_mem_block_coarsed_flipped(M, N, K, alpha, A, B, beta, C);
    break;
default:
...
int main(int argc, char **argv) {
    ...
    //range_init_matrix(C, max_size*max_size);
    ...
    for (int size : SIZE) {
        ...
        //run_sgemm_shared_mem_1d_blocktiling(m, n, k, alpha, dA, dB, beta, dC);
        //run_sgemm_shared_mem_block_coarsed(m, n, k, alpha, dA, dB, beta, dC);
